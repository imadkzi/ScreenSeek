...
export const FilmPage = () => {
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/movie/${location.state}?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-USregion=GB&append_to_response=release_dates,videos,credits,recommendations`
    )
      .then((res) => res.json())
      .then((data) => {
        if (!data.errors) {
          ...
          setCertification(
            data.release_dates.results.filter((x) => x.iso_3166_1 === "GB")[0]
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/movie/${location.state}/watch/providers?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-US&region=GB`
    )
      .then((res) => res.json())
      .then((data) => {
        if (!data.errors) {
          ...
          <div className="details-content__info">
            ...
            <div className="card-btn">
              <div className="icons center">
                <a
                  className="details icon-btn me-2"
                  target="__blank"
                  href={`https://www.youtube.com/watch?v=${trailer}`}
                >
                  <FontAwesomeIcon className="play" icon={faPlay} /> Trailer
                </a>
                <NavLink exact={true} to="/favourites">
                  {" "}
                  <button
                    onClick={() => addToFavourites(movie)}
                    disabled={favouritesDisabled}
                    className="icon-btn details fp"
                  >
                    <FontAwesomeIcon icon={faHeart} />{" "}
                  </button>{" "}
                </NavLink>
                <NavLink exact={true} to="/watchlist">
                  {" "}
                  <button
          ...
          <div className="scroll-container__content scroller">
            {castResults.slice(0, 22).map((cast) => (
              <Nav.Link
                ...
                exact={true}
                to={{
                  pathname: `/cast/${cast.id}-${cast.name}`,
                  state: cast.id,
                }}
